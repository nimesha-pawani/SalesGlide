...
<body>
	...
	<div class="wrapper">
		...
		<div class="main" style="margin-top: 8%;">
			<center>
				<div class="box">
					<h3 align="center" style="margin-top: 20px;margin-bottom:22px;">Sales Statistics</h3>
					<hr>
					&nbsp;&nbsp;<b>Today:&nbsp;&nbsp;</b>
					<?php
					...
					$month = date('m');
					$year = date('Y');
					$sql = "SELECT sum(total_price) AS dailyTotal FROM `sales` WHERE `day`='$day' AND `month`='$month' AND `year`='$year'";
					$result = $con->query($sql);
					while ($row = $result->fetch_array()) {
						echo number_format($row['dailyTotal']);
					}
					?>
					LKR<br><br>
					&nbsp;&nbsp;<b>This Month:&nbsp; </b>
					...
					$month = date('m');
					$year = date('Y');
					$sql = "SELECT sum(total_price) AS monthlyTotal FROM `sales` WHERE `month`='$month' AND `year`='$year'";
					$result = $con->query($sql);
					while ($row = $result->fetch_array()) {
						echo number_format($row['monthlyTotal']);
					}
					?>
					LKR<br><br>
					&nbsp;&nbsp;<b>This Year:&nbsp;</b>
					...
					$month = date('m');
					$year = date('Y');
					$sql = "SELECT sum(total_price) AS yearlyTotal FROM `sales` WHERE  `year`='$year'";
					$result = $con->query($sql);
					while ($row = $result->fetch_array()) {
						echo number_format($row['yearlyTotal']);
					}
					?>
					LKR<br><br>
				</div>
			</center><br>
			<h3 style="margin-top:2px;margin-bottom: 22px;" align="left">Generate Report</h3>
			<center>
				<div class="form-holder">
					<form class="form-inline" style="margin-bottom:8px;" action="<?php htmlspecialchars($_SERVER['PHP_SELF']); ?>" method="POST" class="form-inline" role="form">
						<div class="">
